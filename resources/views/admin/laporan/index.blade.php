<x-app-layout>
    ...
    <div class="py-8 sm:py-12 max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('admin.laporan.index') }}" method="GET"
                ...
                <div class="w-full md:w-1/3">
                    ...
                    <input type="date" name="tanggal_awal" value="{{ request('tanggal_awal', $tanggal_awal) }}"
                        ...
                        class="w-full rounded-2xl border-zinc-200 bg-zinc-50 py-3 px-4 focus:border-[#FCBF49] focus:ring-[#FCBF49] transition duration-200 text-zinc-800 font-medium cursor-pointer"
                        required>
                </div>
                <div class="w-full md:w-1/3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div
                class="bg-zinc-900 border border-zinc-800 p-6 sm:p-8 rounded-3xl shadow-lg relative overflow-hidden group">
                <div
                    class="absolute -right-10 -top-10 w-40 h-40 bg-[#FCBF49] rounded-full opacity-10 blur-3xl group-hover:opacity-20 transition-opacity duration-500">
                </div>
                <h3 class="text-zinc-400 font-bold text-sm uppercase tracking-wider mb-2">Total Pendapatan</h3>
                <p class="text-4xl font-black text-[#FCBF49] tracking-tight">Rp
                    {{ number_format($totalPendapatan, 0, ',', '.') }}</p>
                <p class="text-sm text-zinc-500 mt-3 font-medium">Berdasarkan pesanan selesai pada rentang tanggal
                    terpilih.</p>
            </div>
            ...
            <div class="bg-white border border-zinc-200 p-6 sm:p-8 rounded-3xl shadow-sm relative overflow-hidden">
                ...
                <p class="text-4xl font-black text-zinc-900 tracking-tight">{{ $totalPesanan }} <span
                        class="text-xl text-zinc-400 font-bold">Transaksi</span></p>
                <p class="text-sm text-zinc-500 mt-3 font-medium">Telah dieksekusi dan diselesaikan oleh tim produksi.
                </p>
            </div>
        </div>
            ...
            <div class="overflow-x-auto rounded-2xl border border-zinc-200">
                <table class="min-w-full divide-y divide-zinc-200">
                    <thead class="bg-zinc-50">
                        <tr>
                            <th scope="col"
                                class="px-6 py-4 text-left text-xs font-bold text-zinc-500 uppercase tracking-wider">ID
                                Pesanan</th>
                            <th scope="col"
                            ...
                            <th scope="col"
                                ...
                                Paket Layanan</th>
                            <th scope="col"
                                class="px-6 py-4 text-left text-xs font-bold text-zinc-500 uppercase tracking-wider">Tgl
                                Pesan</th>
                            <th scope="col"
                                class="px-6 py-4 text-right text-xs font-bold text-zinc-500 uppercase tracking-wider">
                                Nominal</th>
                        </tr>
                    </thead>
